...
//------------------Global Variables 
var cols = 50; //the number of columns
var rows = 50; //the number of rows 
var grid = new Array(cols); //the grid which is an array of columns 
...
function setup() {
 ...
 //creating the points so the grid knows where the points are 
  for (var i = 0; i < cols; i++) {
...
//the function that removes previously checked spots from the list 
  ...
  for (var i = 0; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      grid[i][j].show(color(218, 100, 22));
    }
  }
